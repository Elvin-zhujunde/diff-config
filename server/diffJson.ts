...
export function diffJson(
  ...
  businessKey: string = '',
  parentBusinessPath = '',
``  meta: { nextParamsName?: string; primaryKey?: string } = {}
): DiffResult[] {
  ...
  if (Array.isArray(oldData) && Array.isArray(newData)) {
    ...
    if (
      primaryKey &&
      oldData.every(isObject) &&
      newData.every(isObject) &&
      oldData.concat(newData).every(item => primaryKey in item)
    ) {
      // 主键对齐
      ...
      for (const key of oldMap.keys()) {
        if (!newMap.has(key)) {
          diffs.push({
            ...
            pathReadable: parentReadablePath ? `${parentReadablePath} > [${key}]` : `[${key}]`,
            pathBusiness: getBusinessPath(parentBusinessPath, `[${key}]`, oldMap.get(key)),
          } as any);
        }
      }
      // 新增
      for (const key of newMap.keys()) {
        if (!oldMap.has(key)) {
          diffs.push({
            ...
            pathReadable: parentReadablePath ? `${parentReadablePath} > [${key}]` : `[${key}]`,
            pathBusiness: getBusinessPath(parentBusinessPath, `[${key}]`, newMap.get(key)),
          } as any);
        }
      }
      // 修改
      for (const key of newMap.keys()) {
        if (oldMap.has(key)) {
          const children = diffJson(
            ...
            ignoreFields,
            businessKey,
            getBusinessPath(parentBusinessPath, `[${key}]`, newMap.get(key)),
            meta
          );
          if (children.length > 0) {
            const node: DiffResult = {
